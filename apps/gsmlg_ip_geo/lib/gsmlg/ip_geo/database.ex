defmodule GSMLG.IpGeo.Database do
  @moduledoc """
  ...
  Returns the raw MMDB data structure. Use `GSMLG.IpGeo.lookup/2` for
  a normalized result.
  """
  ...
  defp load_default_databases do
    ...
    configured_databases =
      Application.get_env(:gsmlg_ip_geo, :databases, @default_databases)
    ...
    Enum.each(configured_databases, fn {name, filename} ->
      ...
      case do_load(name, path) do
        :ok ->
          Logger.info("[GSMLG.IpGeo] Loaded database", database: name, path: path)

        {:error, :enoent} ->
          Logger.warning("[GSMLG.IpGeo] Database file not found", path: path)

        {:error, reason} ->
          Logger.error("[GSMLG.IpGeo] Failed to load database",
            database: name,
