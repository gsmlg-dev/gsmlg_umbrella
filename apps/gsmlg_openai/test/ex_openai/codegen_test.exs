defmodule GSMLGOpenAITest do
  use ExUnit.Case, async: true

  # in the official openapi docs, causing unknown atoms to be created
  describe "type_to_spec" do
    test "basic types" do
      assert GSMLGOpenAI.Codegen.type_to_spec("number") == {:float, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec(:number) == {:float, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec("integer") == {:integer, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec(:integer) == {:integer, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec("boolean") == {:boolean, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec(:boolean) == {:boolean, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec("bitstring") == {:bitstring, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec(:bitstring) == {:bitstring, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec("pid") == {:pid, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec(:pid) == {:pid, [], []}

      assert GSMLGOpenAI.Codegen.type_to_spec("string") ==
               {{:., [], [{:__aliases__, [alias: false], [:String]}, :t]}, [], []}
    end

    test "enum" do
      assert GSMLGOpenAI.Codegen.type_to_spec({:enum, [:hello, :world, :again]}) ==
               {:|, [], [:again, {:|, [], [:world, :hello]}]}
    end

    test "array" do
      assert GSMLGOpenAI.Codegen.type_to_spec("array") == {:list, [], []}
      assert GSMLGOpenAI.Codegen.type_to_spec({:array, "number"}) == [{:float, [], []}]
    end

    test "object" do
      assert GSMLGOpenAI.Codegen.type_to_spec("object") == {:map, [], []}

      assert GSMLGOpenAI.Codegen.type_to_spec({:object, %{"a" => "number"}}) ==
               {:%{}, [], [{:a, {:float, [], []}}]}
    end

    test "array in object" do
      assert GSMLGOpenAI.Codegen.type_to_spec({:object, %{"a" => {:array, "integer"}}}) ==
               {:%{}, [], [{:a, [{:integer, [], []}]}]}
    end

    test "component" do
      assert GSMLGOpenAI.Codegen.type_to_spec({:component, "Foobar"}) ==
               {{:., [],
                 [{:__aliases__, [alias: false], [:GSMLGOpenAI, :Components, :Foobar]}, :t]}, [],
                []}
    end
    ...
    test "complex nesting" do
      ...
      assert GSMLGOpenAI.Codegen.type_to_spec(sp) ==
               {
                 :%{},
                 [],
                 [
                     ...
                     {:%{}, [],
                      [
                        ...
                        c: [
                          {{:., [],
                            [
                              {:__aliases__, [alias: false], [:GSMLGOpenAI, :Components, :Foo]},
                              :t
                            ]}, [], []}
                        ]
                      ]}
  ...
  test "string_to_component" do
    assert GSMLGOpenAI.Codegen.string_to_component("Hello") == GSMLGOpenAI.Components.Hello
  end

  test "keys_to_atoms" do
    assert GSMLGOpenAI.Codegen.keys_to_atoms(%{
             "a" => 123,
             "b" => %{
               "c" => 23,
               "d" => 456
  ...
  describe "parse_component_schema" do
    test "simple component schema" do
      ...
      parsed = GSMLGOpenAI.Codegen.parse_component_schema(test_schema)
  ...
  describe "parse_type" do
    test "simple type" do
      assert GSMLGOpenAI.Codegen.parse_type(%{
               "type" => "string"
             }) == "string"
    end
  ...
  describe "parse_path" do
    ...
    get:
      ...
      assert GSMLGOpenAI.Codegen.parse_path("/foo/${engine_id}", handler_schema, %{}) == expected
    end
    ...
    post:
      ...
      assert GSMLGOpenAI.Codegen.parse_path("/foo/${engine_id}", handler_schema, comp_mapping) ==
               expected
    end
