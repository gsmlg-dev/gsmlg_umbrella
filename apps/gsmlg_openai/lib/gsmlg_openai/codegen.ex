defmodule GSMLGOpenAI.Codegen do
        ...
        def unpack_ast(partial_tree \\ %{}) do
          ...
          case Enum.member?(resolved_mods, __MODULE__) do
            ...
            false ->
              res =
                @typespec
                # walk through the AST and find all "GSMLGOpenAI.Components"
                ...
                |> Macro.prewalk(partial_tree, fn args, acc ->
                  r =
                    with {:__aliases__, [alias: false], alias} <- args,
                         ...
                         true <- String.contains?(ats, "GSMLGOpenAI.Components") do
                      tree =
                        mod.unpack_ast(%{
  ...
  """
  def module_overwrites, do: [GSMLGOpenAI.Components.Model]
  ...
  def string_to_component(comp), do: Module.concat(GSMLGOpenAI.Components, comp)
