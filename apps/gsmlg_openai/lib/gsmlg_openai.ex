defmodule GSMLGOpenAI do
  ...
  alias GSMLGOpenAI.Config

  def start(_type, _args) do
    children = [Config]
    opts = [strategy: :one_for_one, name: GSMLGOpenAI.Supervisor]
...
docs = GSMLGOpenAI.Codegen.get_documentation()

# Generate structs from schema
docs
|> Map.get(:components)
# generate module name: GSMLGOpenAI.Components.X
|> Enum.map(fn {name, c} ->
  {name
   |> GSMLGOpenAI.Codegen.string_to_component(), c}
end)
# ignore stuff that's overwritten
|> Enum.filter(fn {name, _c} -> name not in GSMLGOpenAI.Codegen.module_overwrites() end)
|> Enum.each(fn {name, component} ->
      ...
      Enum.reduce(
        ...
        fn item, acc ->
          ...
          case kind do
            :required ->
              Map.merge(acc, %{
                String.to_atom(name) => quote(do: unquote(GSMLGOpenAI.Codegen.type_to_spec(type)))
              })

            :optional ->
              Map.merge(acc, %{
                String.to_atom(name) =>
                  quote(do: unquote(GSMLGOpenAI.Codegen.type_to_spec(type)) | nil)
              })
          end
        end
      )
  ...
  defmodule name do
    ...
    use GSMLGOpenAI.Codegen.AstUnpacker
  end
  ...
  defmodule modname do
    ...
    |> Enum.each(fn fx ->
      ...
      merged_optional_args =
        ...
        |> GSMLGOpenAI.Codegen.add_stream_to_opts_args()
        |> Kernel.++(GSMLGOpenAI.Codegen.extra_opts_args())
      ...
      spec =
        merged_required_args
        |> Enum.map(fn item -> quote do: unquote(GSMLGOpenAI.Codegen.type_to_spec(item.type)) end)

      # convert optional args into keyword list
      response_spec = GSMLGOpenAI.Codegen.type_to_spec(response_type)
            ...
            [] ->
              quote do:
                      {unquote(String.to_atom(name)),
                       unquote(GSMLGOpenAI.Codegen.type_to_spec(type))}
      ...
      @doc """
      #{summary |> GSMLGOpenAI.Codegen.fix_openai_links()}
      ...
      #{required_args_docstring |> GSMLGOpenAI.Codegen.fix_openai_links()}


      ### Optional Arguments:

      #{optional_args_docstring |> GSMLGOpenAI.Codegen.fix_openai_links()}
      """
            ...
            {:ok, res} ->
              case unquote(response_type) do
                {:component, comp} ->
                  ...
                  GSMLGOpenAI.Codegen.string_to_component(comp).unpack_ast()
                  ...
                  {:ok,
                   struct(
                     GSMLGOpenAI.Codegen.string_to_component(comp),
                     GSMLGOpenAI.Codegen.keys_to_atoms(res)
                   )}
