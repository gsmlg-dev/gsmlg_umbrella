...
[![Hex.pm Version](https://img.shields.io/hexpm/v/gsmlg_openai)](https://hex.pm/packages/gsmlg_openai)
[![Hex Docs](https://img.shields.io/badge/hex-docs-lightgreen.svg)](https://hexdocs.pm/gsmlg_openai)
[![Hex.pm Download Total](https://img.shields.io/hexpm/dt/gsmlg_openai)](https://hex.pm/packages/gsmlg_openai)

GSMLGOpenAI is an (unofficial) Elixir SDK for interacting with the [OpenAI APIs](https://platform.openai.com/docs/api-reference/introduction)
...
Add **_:gsmlg_openai_** as a dependency in your mix.exs file.
...
```elixir
GSMLGOpenAI.Models.list_models(openai_api_key: "abc", openai_organization_key: "def")
```

## Usage

Make sure to checkout the docs: https://hexdocs.pm/gsmlg_openai

```elixir
GSMLGOpenAI.Models.list_models
{:ok,
 %{
   data: [
     %{
...
```elixir
GSMLGOpenAI.Completions.create_completion "text-davinci-003", prompt: "The sky is"
{:ok,
 %{
   choices: [
     %{
...
{:ok, res} =
  GSMLGOpenAI.Chat.create_chat_completion(msgs, "gpt-3.5-turbo",
    logit_bias: %{
      "8043" => -100
    }
  )
...
{:ok, res} = GSMLGOpenAI.Images.create_image_variation(duck)
...
Create a new client for receiving the streamed data with `use GSMLGOpenAI.StreamingClient`. You'll have to implement the `@behaviour GSMLGOpenAI.StreamingClient`:
...
GSMLGOpenAI.Completions.create_completion "text-davinci-003", prompt: "hello world", stream: true, stream_to: pid
```
