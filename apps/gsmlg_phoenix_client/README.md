...
Add `gsmlg_phoenix_client` and a json library as dependencies in your `mix.exs` file.
`jason` is specified as the default json library.

```elixir
def deps do
  [
    {:gsmlg_phoenix_client, "~> 0.3"},
    {:jason, "~> 1.0"}
  ]
end
```
...
```elixir
  socket_opts =
    Application.get_env(:gsmlg_phoenix_client, :socket)
...
```elixir
  socket_1_opts =
    Application.get_env(:gsmlg_phoenix_client, :socket_1)
  socket_2_opts =
    Application.get_env(:gsmlg_phoenix_client, :socket_2)
