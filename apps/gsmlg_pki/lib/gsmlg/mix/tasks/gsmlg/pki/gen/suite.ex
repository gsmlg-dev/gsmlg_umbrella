defmodule Mix.Tasks.Gsmlg.Pki.Gen.Suite do
  ...
  Generates a suite of test certificates.
      ...
      mix gsmlg.pki.gen.suite test.local
  ...
  default domain from `GSMLG.PKI.Test.Suite` will be used.
  ...
  def run(all_args) do
    ...
    suite = GSMLG.PKI.Test.Suite.new(suite_opts)

    server_key_pem = GSMLG.PKI.PrivateKey.to_pem(suite.server_key)
    create_file(Path.join(path, "server_key.pem"), server_key_pem, force: force)

    if password do
      server_key_enc_pem = GSMLG.PKI.PrivateKey.to_pem(suite.server_key, password: password)
      create_file(Path.join(path, "server_key_enc.pem"), server_key_enc_pem, force: force)
    end

    other_key_pem = GSMLG.PKI.PrivateKey.to_pem(suite.other_key)
    create_file(Path.join(path, "other_key.pem"), other_key_pem, force: force)

    if password do
      other_key_enc_pem = GSMLG.PKI.PrivateKey.to_pem(suite.other_key, password: password)
      create_file(Path.join(path, "other_key_enc.pem"), other_key_enc_pem, force: force)
    end

    client_key_pem = GSMLG.PKI.PrivateKey.to_pem(suite.client_key)
    create_file(Path.join(path, "client_key.pem"), client_key_pem, force: force)

    if password do
      client_key_enc_pem = GSMLG.PKI.PrivateKey.to_pem(suite.client_key, password: password)
      create_file(Path.join(path, "client_key_enc.pem"), client_key_enc_pem, force: force)
    end

    cacerts_pem =
      suite.cacerts
      |> Enum.map(&GSMLG.PKI.Certificate.from_der!/1)
      |> Enum.map(&GSMLG.PKI.Certificate.to_pem/1)
      |> Enum.join()
