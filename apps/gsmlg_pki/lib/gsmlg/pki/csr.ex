defmodule GSMLG.PKI.CSR do
  ...
  alias GSMLG.PKI.RDNSequence
  alias GSMLG.PKI.Certificate.Extension

  @typedoc """
  `:CertificationRequest` record , as used in Erlang's `:public_key` module
  """
  @opaque t :: GSMLG.PKI.ASN1.record(:certification_request)
  ...
  * `:extension_request` - a list of certificate extensions to be included as
    an `extensionRequest` attribute (see `GSMLG.PKI.Certificate.Extension`)
  * `:public_key` - the public key to include in the CSR; by default the public
  ...
  ## Example:
    ...
    ```
    iex> priv = GSMLG.PKI.PrivateKey.new_ec(:secp256r1)
    iex> csr = GSMLG.PKI.CSR.new(priv, "/C=US/ST=NT/L=Springfield/O=ACME Inc.",
    ...>   extension_request: [
    ...>     GSMLG.PKI.Certificate.Extension.subject_alt_name(["www.example.net"])
    ...>   ]
    ...> )
    iex> GSMLG.PKI.CSR.valid?(csr)
    true
    ```

  """
  @spec new(
          GSMLG.PKI.PrivateKey.t() | :crypto.engine_key_ref(),
  ...
  def new(private_key, subject, opts \\ []) do
    ...
    public_key =
      Keyword.get_lazy(opts, :public_key, fn ->
        GSMLG.PKI.PublicKey.derive(private_key)
      end)

    algorithm =
      GSMLG.PKI.SignatureAlgorithm.new(
        hash,
        private_key,
    ...
    info =
      certification_request_info(
        ...
        subjectPKInfo:
          GSMLG.PKI.PublicKey.wrap(public_key, :CertificationRequestInfo_subjectPKInfo),
  ...
  """
  @spec extension_request(t()) :: GSMLG.PKI.RDNSequence.t()
