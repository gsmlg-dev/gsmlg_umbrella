defmodule GSMLG.PKI.Test.Server do
  @moduledoc """
  Simple TLS server for hosting `GSMLG.PKI.Test.Suite` scenarios.
  """
  use GenServer
  ...
  * `:response` - the data to send back to clients when a successful connection
  ...
  """
  @spec start_link({GSMLG.PKI.Test.Suite.t(), Keyword.t()}) :: GenServer.on_start()
  def start_link({suite, opts}) do
    GenServer.start_link(__MODULE__, [suite, opts])
  end

  @doc """
  Returns the TCP port number on which the specified GSMLG.PKI.Test.Server instance
  is listening.
  """
  ...
  defp worker(socket, suite, response) do
    ...
    default_cert = GSMLG.PKI.Certificate.to_der(suite.valid)
    default_key = {:PrivateKeyInfo, GSMLG.PKI.PrivateKey.to_der(suite.server_key, wrap: true)}
    ...
    opts =
      [
        ...
        sni_fun: GSMLG.PKI.Test.Suite.sni_fun(suite),
