defmodule GSMLG.PKI.Test.CRLServer do
  @moduledoc """
  ...
  * `:port` - the TCP port to listen on; defaults to 0, meaning an ephemeral
  ...
  """
  @spec put_crl(pid(), String.t(), GSMLG.PKI.CRL.t()) :: :ok
    ...
    case :gen_tcp.recv(socket, 0) do
      ...
      {:ok, :http_eoh} ->
        case Map.get(crl_map, path) do
          nil ->
            GSMLG.PKI.Logger.warn("No CRL defined for #{path}")
            respond(socket, 404)
            :gen_tcp.close(socket)

          crl ->
            # Logger.info("CRL requested: #{path}")
            respond(socket, 200, GSMLG.PKI.CRL.to_der(crl))
            :gen_tcp.close(socket)
        end
