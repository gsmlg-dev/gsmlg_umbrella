defmodule GSMLG.Web.GaoNoteAttachmentContentController do
  ...
  def show(conn, %{"note_id" => note_id, "path" => segments}) do
    with {:ok, path} <- decoded_wildcard_path(conn, segments),
         {:ok, attachment} <- GaoNote.get_attachment_by_path(note_id, path),
  ...
  defp log_storage_error(note_id, path, reason) do
    GSMLG.Telemetry.error("GaoNote attachment content stream failed",
      metadata: %{
        module: __MODULE__,
        note_id: note_id,
        path: path,
