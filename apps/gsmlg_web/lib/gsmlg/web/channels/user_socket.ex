defmodule GSMLG.Web.UserSocket do
  ...
  channel("node:lobby", GSMLG.Web.NodeChannel)
  channel("room:chess", GSMLG.Web.ChessChannel)
  ...
  def connect(%{"token" => token} = _params, socket, connect_info) do
    ...
    case Guardian.Phoenix.Socket.authenticate(socket, GSMLG.Web.Guardian, token) do
      {:ok, authed_socket} ->
        {:ok, authed_socket}
  ...
  def connect(%{"_csrf_token" => csrf_token}, socket, connect_info) do
    ...
    with {:ok, resource, claims} <-
           Guardian.resource_from_token(GSMLG.Web.Guardian, token, claims_to_check, []) do
  ...
  #
  #     Elixir.GSMLG.Web.Endpoint.broadcast("user_socket:#{user.id}", "disconnect", %{})
  #
