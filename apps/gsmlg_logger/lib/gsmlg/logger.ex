defmodule GSMLG.Logger do
  @moduledoc """
  A collection of formatters and utilities for JSON-based logging for various cloud tools and platforms.

  ## Supported formatters

  * `GSMLG.Logger.Formatters.Basic` - a basic JSON formatter that logs messages in a structured format,
  can be used with any JSON-based logging system, like ElasticSearch, Logstash, etc.

  * `GSMLG.Logger.Formatters.GoogleCloud` - a formatter that logs messages in a structured format that can be
  consumed by Google Cloud Logger and Google Cloud Error Reporter.

  * `GSMLG.Logger.Formatters.Datadog` - a formatter that logs messages in a structured format that can be consumed
  by Datadog.

  * `GSMLG.Logger.Formatters.Elastic` - a formatter that logs messages in a structured format that conforms to the
  ...
  Then, enable the formatter in your `config.exs`:
      ...
      config :logger, :default_handler,
        formatter: {GSMLG.Logger.Formatters.Basic, []}
  ...
  For example in `config.exs`:
      ...
      config :logger, :default_handler,
        formatter: {GSMLG.Logger.Formatters.GoogleCloud, metadata: :all, project_id: "logger-101"}
  ...
  Some formatters require additional configuration options. Here are the options that are common for each formatter:
    ...
    * `:redactors` - a list of tuples, where first element is the module that implements the `GSMLG.Logger.Redactor` behaviour,
