defmodule GSMLG.Openai.Chatgpt do
  use GenServer
  ...
  defp new_msg(m) do
    %GSMLGOpenAI.Components.ChatCompletionRequestMessage{
      content: m,
      role: :user,
      name: "user"
    }
  ...
  @spec to_domain(GSMLGOpenAI.Components.ChatCompletionResponseMessage.t()) :: Message.t()
  ...
  @spec from_domain(Message.t()) :: GSMLGOpenAI.Components.ChatCompletionRequestMessage.t()
  ...
  def handle_call({:msg, m, streamer_pid, "davinci"} = params, from, state) do
    ...
    with msgs <- state.messages ++ [new_msg(m)] do
      ...
      default_prompt_tokens = GSMLG.Openai.Tokenizer.count_tokens!(default_prompt)
      ...
      prompt =
        ...
        |> Enum.reduce_while("", fn x, acc ->
          with summarized <- x <> "\n\n" <> acc do
            if GSMLG.Openai.Tokenizer.count_tokens!(summarized) + default_prompt_tokens >= 2200 do
              {:halt, acc}
            else
              {:cont, summarized}
            end
      ...
      Logger.debug(
        "prompt size: #{String.length(prompt)} -- #{GSMLG.Openai.Tokenizer.count_tokens!(prompt)} tokens"
      )

      case GSMLGOpenAI.Completions.create_completion("text-davinci-003",
             prompt: prompt,
             temperature: 0.7,
             stream: true,
  ...
  @spec handle_call({:msg, String.t(), pid(), String.t()}, any(), state) ::
          {:reply,
           {:ok, GSMLGOpenAI.Components.ChatCompletionResponseMessage.t()} | {:error, any()},
           state}
    ...
    with msgs <- state.messages ++ [new_msg(m)] do
      ...
      filtered_msgs =
        ...
        |> Enum.reduce_while(%{msgs: [], tokens: 0}, fn msg, acc ->
          with msg_tokens <- GSMLG.Openai.Tokenizer.count_tokens!(msg.content) do
      ...
      |> Map.get(:msgs)
      |> Enum.reverse()
      |> GSMLGOpenAI.Chat.create_chat_completion(model,
        temperature: 0.8,
        stream: true,
        stream_to: streamer_pid
      )
