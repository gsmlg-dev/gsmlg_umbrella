defmodule GSMLG.CommandPlatform.CommandDispatcher do
  ...
  require Logger
  alias GSMLG.CommandPlatform.{AgentRegistry, SessionTracker}
  ...
  """
  def close_pty(session_id, force \\ false) do
    with {:ok, session} <- SessionTracker.find_session(session_id),
         :ok <- validate_agent(session.agent_id) do
      audit_log(:close_pty, session.agent_id, %{session_id: session_id, force: force})

      AgentRegistry.send_to_agent(session.agent_id, {:close_pty, session_id, force})
    end
  end
  ...
  """
  def attach_pty(session_id, controller_pid) do
    with {:ok, session} <- SessionTracker.find_session(session_id),
         :ok <- validate_agent(session.agent_id) do
      SessionTracker.set_controller(session_id, controller_pid)
      audit_log(:attach_pty, session.agent_id, %{session_id: session_id})

      AgentRegistry.send_to_agent(session.agent_id, {:attach_pty, session_id})
    end
  end
  ...
  """
  def detach_pty(session_id) do
    with {:ok, session} <- SessionTracker.find_session(session_id),
         :ok <- validate_agent(session.agent_id) do
      SessionTracker.clear_controller(session_id)
      audit_log(:detach_pty, session.agent_id, %{session_id: session_id})

      AgentRegistry.send_to_agent(session.agent_id, {:detach_pty, session_id})
    end
  end
  ...
  def send_input(session_id, data) do
    with {:ok, session} <- SessionTracker.find_session(session_id),
         :ok <- validate_agent(session.agent_id),
         :ok <- validate_input(data) do
      AgentRegistry.send_to_agent(session.agent_id, {:send_input, session_id, data})
    end
  end
  ...
  """
  def resize_pty(session_id, rows, cols) do
    with {:ok, session} <- SessionTracker.find_session(session_id),
         :ok <- validate_agent(session.agent_id),
         :ok <- validate_dimensions(rows, cols) do
      audit_log(:resize_pty, session.agent_id, %{
        session_id: session_id,
        rows: rows,
        cols: cols
      })

      AgentRegistry.send_to_agent(session.agent_id, {:resize_pty, session_id, rows, cols})
    end
  end
