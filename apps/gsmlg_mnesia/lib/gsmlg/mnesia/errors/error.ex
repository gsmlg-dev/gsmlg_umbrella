defmodule GSMLG.Mnesia.Error do
  alias GSMLG.Mnesia.InvalidOperationError
  alias GSMLG.Mnesia.NoTransactionError
  alias GSMLG.Mnesia.AlreadyExistsError
  alias GSMLG.Mnesia.DoesNotExistError
  ...
  # Raise a GSMLG.Mnesia.Error
  defmacro raise(message \\ @default_message) do
    quote do
      raise GSMLG.Mnesia.Error, message: unquote(message)
    end
  end

  # Finds the appropriate GSMLG.Mnesia error from an Mnesia exit
  # Falls back to a default 'MnesiaException'
  defmacro raise_from_code(data) do
    quote(bind_quoted: [data: data]) do
      raise GSMLG.Mnesia.Error.normalize(data)
    end
  end
  ...
  # Helper Method to Build GSMLG.Mnesia Exceptions
  ...
  defp do_normalize(reason) do
    case reason do
      # Mnesia Error Codes
      :no_transaction ->
        %NoTransactionError{message: "Not inside a GSMLG.Mnesia Transaction"}
