defmodule GSMLG.Mnesia.Table do
  ...
  You can define an Mnesia Table by calling `use GSMLG.Mnesia.Table` with a few
  ...
  - `autoincrement` - If the table is of the type `:ordered_set`, setting this
    ...
    when writing records (using `GSMLG.Mnesia.Query.write/2`). Will return an error
  ...
  @typedoc "A GSMLG.Mnesia.Table module"
  @type name :: module()

  @typedoc "A GSMLG.Mnesia.Table record data struct"
  @type record :: struct()
  ...
  Every defined `GSMLG.Mnesia.Table` via the `use` macro, will export this
  ...
  passed module isn't a GSMLG.Mnesia Table.
  ...
  def create_copy(table, node, type) do
    :add_table_copy
    |> GSMLG.Mnesia.Mnesia.call_and_catch([table, node, type])
    |> GSMLG.Mnesia.Mnesia.handle_result()
  end
  ...
  be removed (Mnesia/GSMLG.Mnesia must be stopped first).
  ...
  def delete_copy(table, node) do
    :del_table_copy
    |> GSMLG.Mnesia.Mnesia.call_and_catch([table, node])
    |> GSMLG.Mnesia.Mnesia.handle_result()
  end
  ...
  def move_copy(table, node_from, node_to) do
    :move_table_copy
    |> GSMLG.Mnesia.Mnesia.call_and_catch([table, node_from, node_to])
    |> GSMLG.Mnesia.Mnesia.handle_result()
  end
  ...
  use `GSMLG.Mnesia.Schema.set_storage_type/2` instead.
  ...
  def info(table, key \\ :all) do
    Definition.validate_table!(table)
    GSMLG.Mnesia.Mnesia.call(:table_info, [table, key])
  end
  ...
  def wait(tables, timeout \\ 3000) do
    ...
    GSMLG.Mnesia.Mnesia.call(:wait_for_tables, [tables, timeout])
  end
  ...
  ```
  """
  @spec transform(module(), fun(), list(atom())) ::
          {atom(), :ok} | {:aborted, term()}
  def transform(table, transform_fun, new_attrs) do
    :transform_table
    |> GSMLG.Mnesia.Mnesia.call([table, transform_fun, new_attrs])
    |> GSMLG.Mnesia.Mnesia.handle_result()
  end
  ...
  defp handle_for_bang!(error) do
    GSMLG.Mnesia.Error.raise_from_code(error)
  end
end
