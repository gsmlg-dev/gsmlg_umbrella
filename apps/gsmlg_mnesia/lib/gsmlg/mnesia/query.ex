defmodule GSMLG.Mnesia.Query do
  ...
  that can be executed on GSMLG.Mnesia Tables. Mnesia's "dirty" methods
  are left out on purpose. In almost all circumstances, these
  methods would be enough for interacting with GSMLG.Mnesia Tables, but
  ...
  within the context of a `GSMLG.Mnesia.Transaction`. Outside the
  ...
  ```
  # Will raise an error
  GSMLG.Mnesia.Query.read(Blog.Post, :some_id)
  ...
  # Write a record
  GSMLG.Mnesia.Query.write(%User{id: 3, name: "Some User"})

  # Delete a record by primary key
  GSMLG.Mnesia.Query.delete(User, id)

  # Delete a record by passing the full object
  GSMLG.Mnesia.Query.delete_record(%User{id: 4, name: "Another User"})
  ```


  ## Complex Queries

  GSMLG.Mnesia provides 3 ways of querying records based on some passed
  conditions:
  ...
  ```
  # Get all Movies
  GSMLG.Mnesia.Query.select(Movie, [])

  # Get all Movies named "Rush"
  GSMLG.Mnesia.Query.select(Movie, {:==, :title, "Rush"})
  ...
  GSMLG.Mnesia.Query.select(Movie, guards)
  ```
  """
  ...
  This converts them into simple GSMLG.Mnesia struct records of type
  `t:GSMLG.Mnesia.Table.record/0`. This is equivalent to calling
  `GSMLG.Mnesia.Query.Data.load/1` on the returned records. This option is
  ...
  terminated and executed again. GSMLG.Mnesia does this automatically
  ...
  @doc """
  Finds the GSMLG.Mnesia record for the given id in the specified table.
  ...
  GSMLG.Mnesia.Query.read(Blog.Post, 2, lock: :write)
  # => %Blog.Post{id: 2, ... }
  ...
  @doc """
  Writes a GSMLG.Mnesia record to its Mnesia table.
  ...
  GSMLG.Mnesia record is `nil` and the table does not have autoincrement
  ...
  the table definition. (See `GSMLG.Mnesia.Table` for more details).


  ## Examples

  ```
  GSMLG.Mnesia.Query.write(%Blog.Post{title: "something", ... })
  ...
  ```
  # Both are equivalent
  GSMLG.Mnesia.Query.all(Movie)
  GSMLG.Mnesia.Query.match(Movie, {:_, :_, :_, :_})
  ```
  """
  ...
  This method takes the name of a `GSMLG.Mnesia.Table` and a tuple pattern
  ...
  GSMLG.Mnesia.Query.match(Movie, {:_, "Rush", :_, :_}, lock: :write)

  # Get all movies directed by Tarantino
  GSMLG.Mnesia.Query.match(Movie, {:_, :_, :_, "Quentin Tarantino"})

  # Get all movies directed by Spielberg, in the year 1993
  GSMLG.Mnesia.Query.match(Movie, {:_, :_, 1993, "Steven Spielberg"})

  # Will raise exceptions
  GSMLG.Mnesia.Query.match(Movie, {:_, :_})
  GSMLG.Mnesia.Query.match(Movie, {:_, :_, :_})
  GSMLG.Mnesia.Query.match(Movie, {:_, :_, :_, :_, :_})
  ```
  """
  ...
  GSMLG.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}])
  # => [%Movie{...}, ...]
  ```
  ...
  GSMLG.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: false)
  ...
  GSMLG.Mnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: true)
  # => [%Movie{...}, ...]
  ```
  ...
  This method takes a `GSMLG.Mnesia.Table` name and a key, and deletes all
  ...
  @doc """
  Delete the given GSMLG.Mnesia record object.

  This method accepts a `t:GSMLG.Mnesia.Table.record/0` object and deletes
  ...
  defp validate_match_pattern!(table, pattern) do
    ...
    unless same_size? do
      GSMLG.Mnesia.Error.raise("Match Pattern length is not equal to the no. of attributes")
    end
  end
    ...
    cond do
      ...
      is_nil(primary) ->
        GSMLG.Mnesia.Error.raise(
          "GSMLG.Mnesia records cannot have a nil primary key unless autoincrement is enabled"
        )
    end
  end
