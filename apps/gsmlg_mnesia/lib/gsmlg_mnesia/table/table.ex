defmodule GSMLGMnesia.Table do
  ...
  @moduledoc """
  Defines a GSMLGMnesia Table schema for Mnesia


  ## Usage

  You can define an Mnesia Table by calling `use GSMLGMnesia.Table` with a few
  options in your module.

  ```
  defmodule Blog.Post do
    use GSMLGMnesia.Table, attributes: [:id, :title, :content]
  end
  ```
  ...
  ```
  GSMLGMnesia.Table.create(Blog.Post)
  ```
  ...
  - `autoincrement` - If the table is of the type `:ordered_set`, setting this
    ...
    when writing records (using `GSMLGMnesia.Query.write/2`). Will return an error
  ...
  @typedoc "A GSMLGMnesia.Table module"
  @type name :: module()

  @typedoc "A GSMLGMnesia.Table record data struct"
  @type record :: struct()
  ...
  Every defined `GSMLGMnesia.Table` via the `use` macro, will export this
  ...
  @doc """
  Creates a GSMLGMnesia Table for Mnesia.
  ...
  passed module isn't a GSMLGMnesia Table.
  ...
  def create(table, opts \\ []) do
    ...
    cond do
      ...
      true ->
        ...
        :create_table
        |> GSMLGMnesia.Mnesia.call([table, mnesia_opts])
  ...
  @doc """
  Deletes a GSMLGMnesia Table for Mnesia.
  ...
  GSMLGMnesia.Table.create_copy(Users, :some_node@host_x, :disc_copies)
  ```
  """
  @spec create_copy(name, node, storage_type) :: :ok | {:error, any}
  def create_copy(table, node, type) do
    :add_table_copy
    |> GSMLGMnesia.Mnesia.call_and_catch([table, node, type])
    |> GSMLGMnesia.Mnesia.handle_result()
  end
  ...
  be removed (Mnesia/GSMLGMnesia must be stopped first).
  ...
  def delete_copy(table, node) do
    :del_table_copy
    |> GSMLGMnesia.Mnesia.call_and_catch([table, node])
    |> GSMLGMnesia.Mnesia.handle_result()
  end
  ...
  def move_copy(table, node_from, node_to) do
    :move_table_copy
    |> GSMLGMnesia.Mnesia.call_and_catch([table, node_from, node_to])
    |> GSMLGMnesia.Mnesia.handle_result()
  end
  ...
  use `GSMLGMnesia.Schema.set_storage_type/2` instead.
  ...
  ```
  GSMLGMnesia.Table.set_storage_type(MyTable, :node@host, :disc_copies)
  ```
  """
  ...
  def info(table, key \\ :all) do
    Definition.validate_table!(table)
    GSMLGMnesia.Mnesia.call(:table_info, [table, key])
  end

  @doc """
  Deletes all entries in the given GSMLGMnesia Table.
  ...
  This method can be accessed directly on the `GSMLGMnesia` module as well.
  ...
  GSMLGMnesia.Table.wait(Movies, :infinity)

  # Wait a maximum of 3 seconds until the two tables are ready
  GSMLGMnesia.wait([TableA, TableB])
  ```
  """
  ...
  def wait(tables, timeout \\ 3000) do
    ...
    GSMLGMnesia.Mnesia.call(:wait_for_tables, [tables, timeout])
  end
  ...
  defp handle_for_bang!(error) do
    GSMLGMnesia.Error.raise_from_code(error)
  end
end
