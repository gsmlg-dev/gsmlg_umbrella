defmodule GSMLGMnesia.Table.Definition do
  require GSMLGMnesia.Error

  @moduledoc false

  # Helper module to build GSMLGMnesia Tables with the `use` macro.
  # This module is used to define GSMLGMnesia Tables at compile
  ...
  """
  @spec build_base(GSMLGMnesia.Table.name(), list) :: tuple
  ...
  value of `GSMLGMnesia.Table`.
  """
  @spec build_map(list) :: list
  def struct_fields(attributes) do
    [{:__meta__, GSMLGMnesia.Table} | attributes]
  end
  ...
  def validate_options!(opts) do
    ...
    case error do
      nil -> :ok
      error -> GSMLGMnesia.Error.raise(error)
    end
  end
  ...
  @doc "Raise error if a given module is not a valid GSMLGMnesia Table"
  @spec validate_table!(module) :: :ok | no_return
  def validate_table!(module) do
    GSMLGMnesia.Table = module.__info__.meta
    :ok
  rescue
    _ ->
      GSMLGMnesia.Error.raise("#{inspect(module)} is not a GSMLGMnesia Table")
  end
