defmodule GSMLGMnesia.Query.Data do
  @moduledoc """
  Helper module that acts as an interface to convert GSMLGMnesia Table
  ...
  ```
  GSMLGMnesia.Query.Data.dump(%MyApp.User{id: 1, name: "Sye"})
  # => {MyApp.User, 1, "Sye"}
  ```
  ...
  ```
  GSMLGMnesia.Query.Data.load({MyApp.User, 2, "Rick"})
  ...
  @doc """
  Convert Mnesia data tuple into GSMLGMnesia Table struct.

  Use this method when reading data from an Mnesia database. The
  data should be in a tuple format, where the first element is
  the Table name (`GSMLGMnesia.Table` definition).

  This will automatically match the the tuple values against the
  table's attributes and convert it into a struct of the GSMLGMnesia
  table you defined.
  """
  @spec load(tuple) :: GSMLGMnesia.Table.record()
  ...
  @doc """
  Convert GSMLGMnesia Table struct into Mnesia data tuple.

  Use this method when writing data to an Mnesia database. The
  argument should be a struct of a previously defined GSMLGMnesia
  table definition, and this will convert it into a tuple
  representing an Mnesia record.
  """
  @spec dump(GSMLGMnesia.Table.record()) :: tuple
