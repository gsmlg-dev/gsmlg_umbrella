defmodule GSMLGMnesia.Query do
  require GSMLGMnesia.Mnesia
  require GSMLGMnesia.Error

  alias GSMLGMnesia.Query
  alias GSMLGMnesia.Mnesia
  alias GSMLGMnesia.Table
  alias GSMLGMnesia.Table.Definition

  @moduledoc """
  Module to read/write from GSMLGMnesia Tables.

  This module provides the most important transactional operations
  that can be executed on GSMLGMnesia Tables. Mnesia's "dirty" methods
  are left out on purpose. In almost all circumstances, these
  methods would be enough for interacting with GSMLGMnesia Tables, but
  ...
  within the context of a `GSMLGMnesia.Transaction`. Outside the
  ...
  ```
  # Will raise an error
  GSMLGMnesia.Query.read(Blog.Post, :some_id)

  # Will work fine
  GSMLGMnesia.transaction fn ->
    GSMLGMnesia.Query.read(Blog.Post, :some_id)
  end
  ```
  ...
  GSMLGMnesia.Query.all(User)

  # Get a specific record by its primary key
  GSMLGMnesia.Query.read(User, id)

  # Write a record
  GSMLGMnesia.Query.write(%User{id: 3, name: "Some User"})

  # Delete a record by primary key
  GSMLGMnesia.Query.delete(User, id)

  # Delete a record by passing the full object
  GSMLGMnesia.Query.delete_record(%User{id: 4, name: "Another User"})
  ```


  ## Complex Queries

  GSMLGMnesia provides 3 ways of querying records based on some passed
  conditions:
  ...
  ```
  # Get all Movies
  GSMLGMnesia.Query.select(Movie, [])

  # Get all Movies named "Rush"
  GSMLGMnesia.Query.select(Movie, {:==, :title, "Rush"})
  ...
  GSMLGMnesia.Query.select(Movie, guards)
  ```
  """
  ...
  This converts them into simple GSMLGMnesia struct records of type
  `t:GSMLGMnesia.Table.record/0`. This is equivalent to calling
  `GSMLGMnesia.Query.Data.load/1` on the returned records. This option is
  ...
  terminated and executed again. GSMLGMnesia does this automatically
  ...
  @doc """
  Finds the GSMLGMnesia record for the given id in the specified table.
  ...
  ```
  GSMLGMnesia.Query.read(Blog.Post, 1)
  # => %Blog.Post{id: 1, ... }

  GSMLGMnesia.Query.read(Blog.Post, 2, lock: :write)
  # => %Blog.Post{id: 2, ... }

  GSMLGMnesia.Query.read(Blog.Post, :unknown_id)
  # => nil
  ```
  """
  ...
  @doc """
  Writes a GSMLGMnesia record to its Mnesia table.
  ...
  GSMLGMnesia record is `nil` and the table does not have autoincrement
  ...
  the table definition. (See `GSMLGMnesia.Table` for more details).


  ## Examples

  ```
  GSMLGMnesia.Query.write(%Blog.Post{title: "something", ... })
  # => %Blog.Post{id: 4, title: "something"}

  GSMLGMnesia.Query.write(%Blog.Author{username: "sye", ... })
  ...
  ```
  # Both are equivalent
  GSMLGMnesia.Query.all(Movie)
  GSMLGMnesia.Query.match(Movie, {:_, :_, :_, :_})
  ```
  """
  ...
  This method takes the name of a `GSMLGMnesia.Table` and a tuple pattern
  ...
  GSMLGMnesia.Query.match(Movie, {:_, "Rush", :_, :_}, lock: :write)

  # Get all movies directed by Tarantino
  GSMLGMnesia.Query.match(Movie, {:_, :_, :_, "Quentin Tarantino"})

  # Get all movies directed by Spielberg, in the year 1993
  GSMLGMnesia.Query.match(Movie, {:_, :_, 1993, "Steven Spielberg"})

  # Will raise exceptions
  GSMLGMnesia.Query.match(Movie, {:_, :_})
  GSMLGMnesia.Query.match(Movie, {:_, :_, :_})
  GSMLGMnesia.Query.match(Movie, {:_, :_, :_, :_, :_})
  ```
  """
  ...
  def select(table, guards, opts \\ []) do
    ...
    guards = GSMLGMnesia.Query.Spec.build(guards, attr_map)
  ...
  GSMLGMnesia.Query.select_raw(Movie, [{match_head, guards, result}])
  # => [%Movie{...}, ...]
  ```
  ...
  GSMLGMnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: false)
  ...
  GSMLGMnesia.Query.select_raw(Movie, [{match_head, guards, result}], coerce: true)
  # => [%Movie{...}, ...]
  ```
  ...
  This method takes a `GSMLGMnesia.Table` name and a key, and deletes all
  ...
  GSMLGMnesia.Query.delete(Blog.Post, 10)
  ```
  """
  ...
  @doc """
  Delete the given GSMLGMnesia record object.

  This method accepts a `t:GSMLGMnesia.Table.record/0` object and deletes
  ...
  defp validate_match_pattern!(table, pattern) do
    ...
    unless same_size? do
      GSMLGMnesia.Error.raise("Match Pattern length is not equal to the no. of attributes")
    end
  end
  ...
  defp prepare_record_for_write!(table, record) do
    ...
    cond do
      ...
      is_nil(primary) ->
        GSMLGMnesia.Error.raise(
          "GSMLGMnesia records cannot have a nil primary key unless autoincrement is enabled"
        )
    end
  end
