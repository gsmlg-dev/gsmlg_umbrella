defmodule GSMLGMnesia.Transaction do
  require GSMLGMnesia.Mnesia
  require GSMLGMnesia.Error
  require GSMLGMnesia.TransactionAborted

  @moduledoc """
  GSMLGMnesia's wrapper around Mnesia transactions. This module exports
  ...
  `GSMLGMnesia` module as the alias `GSMLGMnesia.transaction/1` and
  `GSMLGMnesia.transaction!/1` methods, but if you want to specify a
  ...
  operation = fn ->
    GSMLGMnesia.Query.write(%User{id: 3, name: "New Value"})
  end
  GSMLGMnesia.Transaction.execute_sync(operation, 5)
  ```
  """
  ...
  def execute(function, retries \\ :infinity) do
    :transaction
    |> GSMLGMnesia.Mnesia.call_and_catch([function, retries])
  ...
  @spec inside?() :: boolean
  def inside? do
    GSMLGMnesia.Mnesia.call(:is_transaction)
  end

  @doc """
  Aborts a GSMLGMnesia transaction.
  ...
  `GSMLGMnesia.TransactionAborted` error instead of returning the
  ...
  def abort(reason \\ :no_reason_given) do
    case inside?() do
      ...
      false ->
        GSMLGMnesia.Error.raise_from_code(:no_transaction)
    end
  end
  ...
  # be 'handled' by the `GSMLGMnesia.Mnesia` module before this
  # is called.
  defp handle_result(result) do
    case result do
      ...
      {:error, reason} ->
        GSMLGMnesia.Error.raise("Transaction Failed with: #{inspect(reason)}")
