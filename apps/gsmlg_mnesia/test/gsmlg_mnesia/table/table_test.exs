defmodule GSMLGMnesia.Tests.Table do
  ...
  describe "__using__" do
    ...
    test "works with sigils" do
      defmodule Meta.SupportsSigils do
        use GSMLGMnesia.Table, attributes: ~w[id username]a
      end
    ...
    test "works with module attributes" do
      defmodule Meta.SupportsAttributes do
        ...
        use GSMLGMnesia.Table, attributes: @table_attrs
      end
    ...
    test "raises error for invalid options" do
      assert_raise(GSMLGMnesia.Error, ~r/invalid options/i, fn ->
        defmodule MetaApp.InvalidOptions do
          use GSMLGMnesia.Table, :invalid
        end
      end)
    end

    test "raises error if attributes are not specifed" do
      assert_raise(GSMLGMnesia.Error, ~r/attributes not specified/i, fn ->
    ...
    test "raises error for invalid attributes" do
      assert_raise(GSMLGMnesia.Error, ~r/invalid attributes/i, fn ->
        defmodule MetaApp.InvalidAttributes do
          use GSMLGMnesia.Table, attributes: :invalid
        end
      end)

      assert_raise(GSMLGMnesia.Error, ~r/invalid attributes/i, fn ->
        defmodule MetaApp.InvalidAttributes do
          use GSMLGMnesia.Table, attributes: [1, 2, 3]
        end
      end)
    end

    test "raises error for invalid table type" do
      assert_raise(GSMLGMnesia.Error, ~r/invalid.*type/i, fn ->
  ...
  describe "#create" do
    ...
    test "returns :error if the table already exists" do
      ...
      assert {:error, {:already_exists, _}} = GSMLGMnesia.Table.create(@table)
    end

    test "returns :error if autoincrement is used without :ordered_set (options)" do
      assert {:error, {:autoincrement, message}} =
               GSMLGMnesia.Table.create(@table, autoincrement: true)
    ...
    test "returns :error if autoincrement is used without :ordered_set (definition)" do
      ...
      assert {:error, {:autoincrement, message}} =
               GSMLGMnesia.Table.create(MetaApp.AutoincrementBag)
  ...
  describe "#create!" do
    ...
    test "raises AlreadyExistsError if table already exists" do
      ...
      assert_raise(GSMLGMnesia.AlreadyExistsError, ~r/already exists/i, fn ->
        GSMLGMnesia.Table.create!(@table)
      end)
    end
    ...
    test "raises InvalidOperationError when autoincrement is used with a type other than :ordered_set" do
      ...
      assert_raise(
        GSMLGMnesia.InvalidOperationError,
        ~r/can only be used with.*ordered.set/i,
        fn ->
          GSMLGMnesia.Table.create!(MetaApp.AutoincrementBagBang)
        end
      )
    end
  end
  ...
  describe "#delete" do
    ...
    test "returns :error if the table does not exists" do
      assert_raise(GSMLGMnesia.DoesNotExistError, ~r/does not exist/i, fn ->
        GSMLGMnesia.Table.delete!(@table)
      end)
    end
  end
  ...
  describe "#delete!" do
    ...
    test "raises errors if the table does not exists" do
      assert {:error, {:no_exists, _}} = GSMLGMnesia.Table.delete(@table)
    end
  end
  ...
  describe "#info" do
    ...
    test "returns table information" do
      assert GSMLGMnesia.Table.info(@table) == :mnesia.table_info(@table, :all)
    end

    test "returns specific table information if key is given" do
      assert GSMLGMnesia.Table.info(@table, @key) == :mnesia.table_info(@table, @key)
    end
  end
  ...
  describe "#clear" do
    ...
    test "returns ok and deletes all records in the table" do
      ...
      assert :ok = GSMLGMnesia.Table.clear(@table)
    ...
    test "returns :error if the table does not exists" do
      GSMLGMnesia.Table.delete(@table)
      assert {:error, {:no_exists, _}} = GSMLGMnesia.Table.clear(@table)
    end
  ...
  describe "#callback:__info__" do
    ...
    test "it returns important meta information about the table" do
      ...
      assert info.meta == GSMLGMnesia.Table
    ...
    test "it contains both Mnesia and GSMLGMnesia Options" do
