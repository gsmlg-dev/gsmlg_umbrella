defmodule GSMLGMnesia.Tests.Query do
  ...
  describe "#write" do
    ...
    test "raises error when primary key is nil" do
      assert_raise(GSMLGMnesia.Error, ~r/cannot have a nil primary key/i, fn ->
        Support.Mnesia.transaction(fn ->
  ...
  describe "#match" do
    ...
    test "raises error when the no. of items in the tuple don't match" do
      assert_raise(GSMLGMnesia.Error, ~r/not equal to .* attributes/i, fn ->
        Support.Mnesia.transaction(fn ->
    ...
    test "returns all records that match a specific attribute" do
      Support.Mnesia.transaction(fn ->
        assert [%{title: "Jaws", year: 1975}] = Query.match(@table, {:_, :_, 1975, :_})

        assert [%{title: "Rush"}, %{title: "Rush"}] = Query.match(@table, {:_, "Rush", :_, :_})
      end)
    end
  ...
  describe "#select" do
    ...
    test "returns all records that match a specific attribute" do
      Support.Mnesia.transaction(fn ->
        assert [%{title: "Jaws", year: 1975}] = Query.select(@table, {:==, :year, 1975})

        assert [%{title: "Rush"}, %{title: "Rush"}] = Query.select(@table, {:==, :title, "Rush"})
      end)
    end
