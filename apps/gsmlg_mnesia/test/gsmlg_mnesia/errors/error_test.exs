defmodule GSMLGMnesia.Tests.Error do
  ...
  describe "#raise" do
    ...
    test "raises error with given message" do
      assert_raise(GSMLGMnesia.Error, @message, fn ->
        GSMLGMnesia.Error.raise(@message)
      end)
    end
  end
  ...
  describe "#normalize" do
    test "resolves to AlreadyExistsError for :already_exists" do
      assert %GSMLGMnesia.AlreadyExistsError{} = GSMLGMnesia.Error.normalize({:already_exists, X})
    end

    test "resolves to DoesNotExistError for :no_exists" do
      assert %GSMLGMnesia.DoesNotExistError{} = GSMLGMnesia.Error.normalize({:no_exists, X})
    end

    test "resolves to NoTransactionError for :no_transaction" do
      assert %GSMLGMnesia.NoTransactionError{} = GSMLGMnesia.Error.normalize(:no_transaction)
    end

    test "resolves to InvalidOperationError for :autoincrement" do
      assert %GSMLGMnesia.InvalidOperationError{} =
               GSMLGMnesia.Error.normalize({:autoincrement, "blah blah"})
    end
    ...
    test "falls back to MnesiaException for other errors" do
      Enum.each(@rest_of_mnesia_errors, fn error ->
        assert %GSMLGMnesia.MnesiaException{} = GSMLGMnesia.Error.normalize({error, X})
      end)
    end
  end
end
