defmodule GSMLG.Mnesia.Tests.Table do
  ...
  describe "__using__" do
    ...
    test "works with sigils" do
      defmodule Meta.SupportsSigils do
        use GSMLG.Mnesia.Table, attributes: ~w[id username]a
      end
    ...
    test "works with module attributes" do
      defmodule Meta.SupportsAttributes do
        ...
        use GSMLG.Mnesia.Table, attributes: @table_attrs
      end
    ...
    test "raises error for invalid options" do
      assert_raise(GSMLG.Mnesia.Error, ~r/invalid options/i, fn ->
        defmodule MetaApp.InvalidOptions do
          use GSMLG.Mnesia.Table, :invalid
        end
      end)
    end

    test "raises error if attributes are not specifed" do
      assert_raise(GSMLG.Mnesia.Error, ~r/attributes not specified/i, fn ->
    ...
    test "raises error for invalid attributes" do
      assert_raise(GSMLG.Mnesia.Error, ~r/invalid attributes/i, fn ->
        defmodule MetaApp.InvalidAttributes do
          use GSMLG.Mnesia.Table, attributes: :invalid
        end
      end)

      assert_raise(GSMLG.Mnesia.Error, ~r/invalid attributes/i, fn ->
        defmodule MetaApp.InvalidAttributes do
          use GSMLG.Mnesia.Table, attributes: [1, 2, 3]
        end
      end)
    end

    test "raises error for invalid table type" do
      assert_raise(GSMLG.Mnesia.Error, ~r/invalid.*type/i, fn ->
  ...
  describe "#create" do
    ...
    test "returns :error if autoincrement is used without :ordered_set (definition)" do
      ...
      assert {:error, {:autoincrement, message}} =
               GSMLG.Mnesia.Table.create(MetaApp.AutoincrementBag)

      assert message =~ ~r/can only be used with.*ordered.set/i
    end

    test "raises error if module is not a gsmlg_mnesia table" do
      assert_raise(GSMLG.Mnesia.Error, ~r/not a GSMLG.Mnesia Table/i, fn ->
        GSMLG.Mnesia.Table.create(RandomModule)
      end)
    end
  end
  ...
  describe "#create!" do
    ...
    test "raises AlreadyExistsError if table already exists" do
      ...
      assert_raise(GSMLG.Mnesia.AlreadyExistsError, ~r/already exists/i, fn ->
        GSMLG.Mnesia.Table.create!(@table)
      end)
    end
    ...
    test "raises InvalidOperationError when autoincrement is used with a type other than :ordered_set" do
      ...
      assert_raise(
        GSMLG.Mnesia.InvalidOperationError,
        ~r/can only be used with.*ordered.set/i,
        fn ->
          GSMLG.Mnesia.Table.create!(MetaApp.AutoincrementBagBang)
        end
      )
    end
  end
  ...
  describe "#delete" do
    ...
    test "raises error if module is not a gsmlg_mnesia table" do
      assert_raise(GSMLG.Mnesia.Error, ~r/not a GSMLG.Mnesia Table/i, fn ->
        GSMLG.Mnesia.Table.delete(RandomModule)
      end)
    end

    test "deletes an mnesia table from gsmlg_mnesia definition" do
      assert {:atomic, :ok} = :mnesia.create_table(@table, [])
      assert :ok = GSMLG.Mnesia.Table.delete!(@table)
    end

    test "returns :error if the table does not exists" do
      assert_raise(GSMLG.Mnesia.DoesNotExistError, ~r/does not exist/i, fn ->
        GSMLG.Mnesia.Table.delete!(@table)
      end)
    end
  end
  ...
  describe "#delete!" do
    ...
    test "raises errors if the table does not exists" do
      assert {:error, {:no_exists, _}} = GSMLG.Mnesia.Table.delete(@table)
    end
  end
  ...
  describe "#info" do
    ...
    setup(do: GSMLG.Mnesia.Table.create(@table))

    test "raises error if module is not a gsmlg_mnesia table" do
      assert_raise(GSMLG.Mnesia.Error, ~r/not a GSMLG.Mnesia Table/i, fn ->
        GSMLG.Mnesia.Table.info(RandomModule)
      end)
    end

    test "returns table information" do
      assert GSMLG.Mnesia.Table.info(@table) == :mnesia.table_info(@table, :all)
    end

    test "returns specific table information if key is given" do
      assert GSMLG.Mnesia.Table.info(@table, @key) == :mnesia.table_info(@table, @key)
    end
  end
  ...
  describe "#clear" do
    ...
    test "raises error if module is not a gsmlg_mnesia table" do
      assert_raise(GSMLG.Mnesia.Error, ~r/not a GSMLG.Mnesia Table/i, fn ->
        GSMLG.Mnesia.Table.clear(RandomModule)
      end)
    end
    ...
    test "returns ok and deletes all records in the table" do
      ...
      assert :ok = GSMLG.Mnesia.Table.clear(@table)
    ...
    test "returns :error if the table does not exists" do
      GSMLG.Mnesia.Table.delete(@table)
      assert {:error, {:no_exists, _}} = GSMLG.Mnesia.Table.clear(@table)
    end

    defp get_first_key do
      {:atomic, key} = :mnesia.transaction(fn -> :mnesia.first(@table) end)
  ...
  describe "#callback:__info__" do
    ...
    test "it returns important meta information about the table" do
      ...
      assert info.meta == GSMLG.Mnesia.Table
    ...
    test "it contains both Mnesia and GSMLG.Mnesia Options" do
